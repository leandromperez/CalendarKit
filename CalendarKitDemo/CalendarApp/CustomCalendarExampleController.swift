...
class CustomCalendarExampleController: DayViewController, DatePickerControllerDelegate {
    ...
    private func generateEventsForDate(_ date: Date) -> [EventDescriptor] {
        ...
        for i in 0...4 {
            ...
            event.isAttributedTextInteractive = false
            ...
            let another = event.copy() as! Event
            let startOffset = TimeChunk(seconds: 0, minutes: .random(in: -45...45), hours: 0, days: 0, weeks: 0, months: 0, years: 0)
            let endOffset = TimeChunk(seconds: 0, minutes: .random(in: 45...120), hours: 0, days: 0, weeks: 0, months: 0, years: 0)
            another.startDate = datePeriod.beginning!.add(startOffset)
            another.endDate = datePeriod.end!.add(endOffset)
            events.append(another)
